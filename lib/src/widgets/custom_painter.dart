...
class ContainerPainter extends CustomUIWidget {
  ...
  void paint(Canvas canvas, Size containerSize) {
    ...
    if (rect.width <= 0 || rect.height <= 0 || rect.hasNaN) {
      print('⚠️ Skipping paint due to invalid rect: $rect');
      return;
    }
    ...
    if (color != null) {
      ...
      if (borderRadius != null && borderRadius!.isNaN) {
        print('⚠️ borderRadius is NaN: $borderRadius');
      }
  ...
  void layout(Size availableSize) {
    ...
    if (children.isNotEmpty) {
      ...
      child.layout(Size(
        ...
        math.max(0, contentArea.height),
      ));
      print('ContainerPainter layout: availableSize=$availableSize, contentArea=$contentArea, child.size=${child.size}');
      size = Size(
        math.max(size.width, child.size.width + (padding?.horizontal ?? 0) + (margin?.horizontal ?? 0)),
        math.max(size.height, child.size.height + (padding?.vertical ?? 0) + (margin?.vertical ?? 0)),
      );
    }
  }
}
...
class CTextPainter extends CustomUIWidget {
  ...
  void layout(Size availableSize) {
    ...
    size = Size(
      _textPainter.width + (padding?.horizontal ?? 0) + (margin?.horizontal ?? 0),
      _textPainter.height + (padding?.vertical ?? 0) + (margin?.vertical ?? 0),
    );
    print('CTextPainter layout: text=$text, size=$size, availableSize=$availableSize');
  }
...
class ColumnPainter extends CustomUIWidget {
  ...
  void layout(Size availableSize) {
    final contentArea = Rect.fromLTWH(
      ...
      math.max(0, availableSize.width - (margin?.horizontal ?? 0) - (padding?.horizontal ?? 0)),
      math.max(0, availableSize.height - (margin?.vertical ?? 0) - (padding?.vertical ?? 0)),
    );

    print('ColumnPainter layout: availableSize=$availableSize, contentArea=$contentArea');
    ...
    if (mainAxisAlignment == MainAxisAlignment.center) {
      ...
      final spacing = (contentArea.height - totalHeight) / (children.length + 1);
      currentY += spacing;
    }
    ...
    for (int i = 0; i < children.length; i++) {
      ...
      if (mainAxisAlignment == MainAxisAlignment.spaceEvenly && i < children.length - 1) {
        final spacing = (contentArea.height - totalHeight) / (children.length + 1);
        currentY += spacing;
      }
    }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomPaint(
          size: canvasSize,
          painter: CustomUIRenderer(child),
        );
      },
    );
  }
