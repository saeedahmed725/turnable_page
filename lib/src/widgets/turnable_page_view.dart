...
class _TurnablePageViewState extends State<TurnablePageView>
  ...
  Future<void> _loadPageContent() async {
    ...
    for (var i = 0; i < widget.pageCount; i++) {
      try {
        ...
        images.add(await _createFallbackImage(i, '$e'));
      }
    }
  ...
  Future<ui.Image> _createFallbackImage(int pageIndex, String message) async {
    final recorder = ui.PictureRecorder();
    final canvas = Canvas(recorder);

    // Draw white background
    canvas.drawRect(
      Rect.fromLTWH(0, 0, widget.settings.width, widget.settings.height),
    ...
    textPainter.paint(
      canvas,
      Offset(
        (widget.settings.width - textPainter.width) / 2,
        (widget.settings.height - textPainter.height) / 2,
      ),
    );
